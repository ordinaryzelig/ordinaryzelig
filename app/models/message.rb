class Message < ActiveRecord::Base
  ...
  can_be_summarized_by :what => :body, :who => :poster, :when => :posted_at, :max => 100, :title => :subject, :type => proc {self.class}, :url => proc { {:controller => "message_board", :action => "show", :id => self.id} }
